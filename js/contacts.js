...
try {
    ...
    (function () {
        ...
        var element = document.querySelector('div.contacts-form input[type="tel"]');
		//  	maskOptions = {
        ...
        function requestSendFeedback(fields) {
            ...
            try {
                return fetch(window.pms.config.feedbackAPI + 'order/create', {
                    method: 'POST',
                    ...
                    if (response.status) {
                        ...
                        var form = document.querySelector('div.contacts-form');
        ...
        document.addEventListener('click', function (event) {
            try {
                ...
                if (event.target.tagName == "IMG" && event.target.closest('button.popup-close') || event.target.tagName == "BUTTON" && event.target.classList.contains('popup-close') || event.target.tagName == "DIV" && event.target.closest('div.popup-fon')) {
                    PopUpHidePopup();
                ...
                if (event.target.tagName == "BUTTON" && event.target.getAttribute('type') == "button" && event.target.closest('div.contacts-form')) {
                    ...
                    if (sendRequest) requestSendFeedback(fieldsData);
                }
            } catch (error) {
				requestSendBugs(error);
			}
		...
		window.onresize = function (event) {
            try {
                var buttonAction = document.querySelector('button[data-action]');
                if (window.innerWidth >= mobileWidth) buttonAction.parentNode.querySelector('.info-text').style.height = "195px"; else {
