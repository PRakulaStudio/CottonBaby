...
function reuestGetCategories(page) {
    ...
    return fetch(window.pms.config.catalogApi + 'categories', {
        ...
        if (response.status) location.reload(); else alert('Не получилось разлогиниться');
    });
}
...
window.onresize = function (event) {
    ...
    if (window.innerWidth > mobileWidth) {
        ...
        document.querySelector('div.title button').style.display = "block";
        if (button.hasAttribute('data-category-action')) displayCategories(button.getAttribute('data-category-action')); else displayCategories('hide');
    }
};
...
function displayCategories(status_display) {
    //если мобилка
    if (window.innerWidth <= mobileWidth) {
        if (status_display == "show") document.querySelector('section.filter-box').style.display = "block"; else document.querySelector('section.filter-box').style.display = "none";
    } else {
        document.querySelector('section.filter-box').style.display = "block";
        if (status_display == "show") document.querySelector('section.filter-box').style.display = "block"; else document.querySelector('section.filter-box').style.display = "none";
    }
...
Promise.all([requestCheckAuth('katalog'), createPagination(pms.plugins.catalog.currentCategory.count, 'katalog')]
